...
fun K5.showTriangularMesh() {
    ...
    fun DrawScope.drawTriangle(p1: Vector2D, p2: Vector2D, p3: Vector2D) {
        ...
        path.close()
        drawPath(path, Color(ltColors[(ltColors.indices).random()]))
    }

    show {

        for (y in (gap / 2)..size.width.toInt() step gap) {
            odd = !odd
            line = mutableListOf()
            for (x in (gap / 2)..size.width.toInt() step gap) {
